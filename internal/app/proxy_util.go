...
type proxyRequestContext struct {
	...
	tokenName     string // Token名称（用于SSE日志实时显示，2025-12新增）
...
// injectAPIKeyHeaders 按路径类型注入API Key头（Gemini vs Claude/Codex）
...
func buildLogEntry(originalModel string, channelID int64, channelName string, channelType string, statusCode int,
	...
	if errMsg != "" {
		...
		msg := truncateErr(errMsg)
		if res != nil && isStreaming {
			if res.FirstByteTime > 0 {
				entry.FirstByteTime = res.FirstByteTime
			}
			if res.BytesReceived > 0 {
				msg = fmt.Sprintf("%s (received %s)", msg, formatBytes(res.BytesReceived))
			}
		}
		entry.Message = msg
	} else if res != nil {
