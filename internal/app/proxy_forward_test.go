...
func TestHandleSuccessResponse_ExtractsUsageFromJSON(t *testing.T) {
	...
	res, _, err := s.handleSuccessResponse(reqCtx, resp, 0, resp.Header.Clone(), rec, "anthropic", &testChannelID, testAPIKey, "/v1/chat/completions", "https://test.example.com/v1/chat/completions", false, nil, false)
	if err != nil {
...
func TestHandleSuccessResponse_StreamDiagMsg_NonAnthropicNoUsage(t *testing.T) {
	...
	res, _, err := s.handleSuccessResponse(reqCtx, resp, 0, resp.Header.Clone(), rec, "codex", &testChannelID, testAPIKey, "/v1/chat/completions", "https://test.example.com/v1/chat/completions", false, nil, false)
	if err != nil {
		t.Fatalf("handleSuccessResponse returned error: %v", err)
	}
