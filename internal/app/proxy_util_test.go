...
func TestBuildLogEntry_StreamDiagMsg(t *testing.T) {
	...
	t.Run("正常成功响应", func(t *testing.T) {
		...
		entry := buildLogEntry("claude-3", channelID, channelName, 200, 1.5, true, "sk-test", "https://api.example.com", 0, "", "", res, "")
		if entry.Message != "ok" {
	...
	t.Run("errMsg优先于StreamDiagMsg", func(t *testing.T) {
		...
		entry := buildLogEntry("claude-3", channelID, channelName, 200, 1.5, true, "sk-test", "https://api.example.com", 0, "", "", res, errMsg)
