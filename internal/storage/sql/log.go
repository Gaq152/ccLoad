...
func normalizeLogAPIKeyFields(e *model.LogEntry) (string, string) {
	...
	if maskedKey[0] == '[' && maskedKey[len(maskedKey)-1] == ']' {
		return maskedKey, ""
	}

	if apiKeyHash == "" {
		apiKeyHash = util.HashAPIKey(maskedKey)
	}

	return maskAPIKey(maskedKey), apiKeyHash
}
