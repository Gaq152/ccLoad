...
func DefineModelPricingTable() *TableBuilder {
	return NewTable("model_pricing").
		...
		Column("channel_type VARCHAR(64) NOT NULL").                // anthropic/codex/gemini
		Column("input_price DOUBLE NOT NULL").                      // $/1M tokens
		Column("output_price DOUBLE NOT NULL").                     // $/1M tokens
		Column("cache_read_price DOUBLE NOT NULL DEFAULT 0").       // 缓存读取 $/1M tokens
		Column("cache_write_price DOUBLE NOT NULL DEFAULT 0").      // 缓存写入 $/1M tokens
		Column("input_price_high DOUBLE NOT NULL DEFAULT 0").       // 长上下文输入价
		Column("output_price_high DOUBLE NOT NULL DEFAULT 0").      // 长上下文输出价
		Column("cache_read_price_high DOUBLE NOT NULL DEFAULT 0").  // 长上下文缓存读取价
		Column("cache_write_price_high DOUBLE NOT NULL DEFAULT 0"). // 长上下文缓存写入价
		Column("high_price_threshold BIGINT NOT NULL DEFAULT 0").   // 0=无分档；大于0时为输入Token阈值
		Column("aliases TEXT DEFAULT ''").                          // 别名列表（逗号分隔）
		Column("is_default TINYINT NOT NULL DEFAULT 0").            // 是否加入渠道默认模型列表
		Column("created_at BIGINT NOT NULL").
		Column("updated_at BIGINT NOT NULL").
