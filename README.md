...
一个高性能的多渠道 API 透明代理服务，支持 **Claude Code**、**Codex CLI**、**Gemini CLI** 官方 OAuth 认证和自动 Token 刷新。使用 Go 和 Gin 框架构建，提供多渠道负载均衡、智能故障切换、多端点管理和实时监控。支持 SQLite 和 MySQL 双存储引擎。
...
### 核心能力
- 🚀 **高性能架构** - 基于 Gin 框架，支持 1000+ 并发连接，异步 Redis 同步响应 <1ms
- 🎯 **智能路由** - 基于优先级和轮询的渠道选择算法，预过滤冷却渠道，支持多 Key 负载均衡
- 🛡️ **故障切换** - 自动失败检测和指数退避冷却机制（2min → 4min → 8min → 30min 上限）
- 🔀 **多端点管理** - 单渠道支持多个 API 端点，自动测速选择最优端点
...
- 🌡️ **健康监控矩阵** - 渠道健康状态可视化，支持分类展示和隐藏管理
- 📈 **趋势分析** - 24 小时请求趋势图，支持暗色/亮色主题切换
- ⏱️ **冷却倒计时** - 渠道冷却时间本地实时倒计时显示
- 🔄 **用量监控** - 渠道 API 用量实时查询，支持多种提取模板

### 令牌管理
- 🎫 **API 令牌系统** - 支持创建多个 API 访问令牌，独立统计用量
- 🔒 **渠道访问控制** - 令牌可限制只能访问指定渠道
- ⏰ **有效期管理** - 支持设置令牌过期时间，支持禁用/启用
- 📋 **日志追踪** - 日志记录显示调用令牌名称，便于用户识别

### 界面体验
- 🌓 **主题切换** - 支持暗色/亮色主题，圆形扩展动画切换效果
- 📁 **分组折叠** - 渠道/令牌页面按类型分组折叠显示
- 🔍 **前缀匹配筛选** - 筛选框支持前缀匹配，交互体验优化
...
#     },
#     {
#       "name": "claude-haiku-4-5-20250101",
