...
function loadFrontendSettings() {
  // 用量查询并发数
  const quotaRequestConcurrency = localStorage.getItem('quota_request_concurrency');
  if (quotaRequestConcurrency) {
    const input = document.getElementById('quota_request_concurrency');
    if (input) {
      input.value = quotaRequestConcurrency;
    }
  }

  // 批量查询并发数
  const quotaBatchConcurrency = localStorage.getItem('quota_batch_concurrency');
  if (quotaBatchConcurrency) {
    const input = document.getElementById('quota_batch_concurrency');
    if (input) {
      input.value = quotaBatchConcurrency;
    }
  }
}
...
function saveFrontendSettings() {
  try {
    // 用量查询并发数
    const quotaRequestInput = document.getElementById('quota_request_concurrency');
    if (quotaRequestInput) {
      let value = parseInt(quotaRequestInput.value);
      if (isNaN(value) || value < 1) value = 1;
      if (value > 50) value = 50;
      quotaRequestInput.value = value; // 更新输入框显示
      localStorage.setItem('quota_request_concurrency', value.toString());
    }

    // 批量查询并发数
    const quotaBatchInput = document.getElementById('quota_batch_concurrency');
    if (quotaBatchInput) {
      let value = parseInt(quotaBatchInput.value);
      if (isNaN(value) || value < 1) value = 1;
      if (value > 50) value = 50;
      quotaBatchInput.value = value; // 更新输入框显示
      localStorage.setItem('quota_batch_concurrency', value.toString());
    }

    showSuccess('前端配置已保存，刷新页面后生效');
  } catch (err) {
    console.error('保存前端配置失败:', err);
    showError('保存前端配置失败: ' + err.message);
  }
}
 ...
 */
function resetFrontendSetting(key, defaultValue) {
  try {
    const input = document.getElementById(key);
    if (input) {
      input.value = defaultValue;
      localStorage.setItem(key, defaultValue.toString());
      showSuccess(`配置 ${key} 已重置为默认值 (${defaultValue})`);
    }
  } catch (err) {
    console.error('重置前端配置失败:', err);
    showError('重置前端配置失败: ' + err.message);
  }
}
